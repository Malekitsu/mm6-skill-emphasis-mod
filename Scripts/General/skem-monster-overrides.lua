...
function calculateMapAverage(map)
	total_levels = 0
	total_monsters = 0
	
	for i=1, Map.Monsters.High
	do
...
function getAdaptiveMultiplier(switch)
	...
	then
		mode = "Map"
		output = calculateMapAverage(map)
	else
		mode = "Unknown"
...
function applyMonsterDamageMultipliers(monsterArray, damageMultiplier, rankMultiplier, easy_flag)
	for i=1,2 do
		key = "Attack" .. i
		resist = monsterArray[key]["Type"]
		sides = monsterArray[key]["DamageDiceSides"]
		bonus = monsterArray[key]["DamageAdd"] 
	...
	if not(monsterArray["SpellChance"] == 0)
	then
		local rank, mastery = SplitSkill(monsterArray["SpellSkill"])
...
function applyAdaptiveMonsterOverrides(monsterID, monsterArray, adaptive_level)
	levelMultiplier = (adaptive_level + 1) / (monsterArray["Level"] + 1)

	dice = monsterArray["Attack1"]["DamageDiceCount"]
	sides = monsterArray["Attack1"]["DamageDiceSides"]
	add = monsterArray["Attack1"]["DamageAdd"]

	newSides = math.max(sides * levelMultiplier, 1)
	newAdd = math.floor(add * levelMultiplier)

	oldDiceString = dice .. "D" .. sides .. "+" .. add
	newDiceString = dice .. "D" .. newSides .. "+" .. newAdd

	monsterArray["FullHP"] = adaptive_level * (3 + (adaptive_level/10)) * baseHealthMultiplier
	monsterArray["HP"] = monsterArray["FullHP"]
	monsterArray["ArmorClass"] = monsterArray["ArmorClass"] * levelMultiplier
	monsterArray["Experience"] = monsterArray["Experience"] * levelMultiplier
	monsterArray["TreasureDiceCount"] = monsterArray["TreasureDiceCount"] * levelMultiplier
	
	monsterArray["Attack1"]["DamageAdd"] = newAdd
	monsterArray["Attack1"]["DamageDiceSides"] = newSides
	
	if (adaptive_level > monsterArray["Level"])
	then
		for k,_ in pairs(resistanceTypes)
		do
			if not (k == "Energy")
			then
				...
				value = value * (adaptive_level + 100)/(monsterArray["Level"] + 100) + (adaptive_level - monsterArray["Level"])/2
	...
	then
		monsterArray["Attack2"]["DamageAdd"] = monsterArray["Attack2"]["DamageAdd"] * levelMultiplier
		monsterArray["Attack2"]["DamageDiceSides"] = math.max(1, monsterArray["Attack2"]["DamageDiceSides"] * levelMultiplier)
	elseif not (monsterArray["SpellChance"] == 0)
	then
		r,m = SplitSkill(monsterArray["SpellSkill"])
		r = math.max(1, r * levelMultiplier)
		monsterArray["SpellSkill"] = JoinSkill(r,m)
	end

	monsterArray["Level"] = math.ceil(adaptive_level)
	for k,v in pairs(monsterArray) do
		if not (type(monsterArray[k]) == table)
		then
